...
/**
 * 建立 Velin Music 首个稳定版本的唯一 SQLite 核心发布基线（NFR-REL-006A、DEV-MIG-003A）。
 *
 * 为什么存在：项目在首发前已经形成大量仅用于开发演进的迁移。2026-09-03 经用户批准完成最终压缩，
 * 本文件把旧基线和后续八段迁移的空库终态合并为一个版本，使新部署只执行一个文件。压缩前所有已知
 * 开发库必须先升级到本文件记录的终态；迁移在既有终态上只收敛 Phinx 账本，不改写业务表或插件对象。
 *
 * 前置条件与不变量：仅支持 SQLite；Phinx 必须在独占迁移事务中调用。空库除 `phinxlog` 外不得存在任何
 * 对象；非空库必须已经与基线的规范化核心 schema SHA-256 完全一致。已安装 PHP 插件可在独立命名空间
 * 创建动态表和索引，这些对象不属于核心基线但必须保留；除明确命名空间外的未知对象仍然失败关闭。
 * 空库只写固定内置主题、角色、能力、默认
 * 音乐库、存储策略、系统设置和九个元数据源，不包含任何开发实例数据或秘密。已存在终态只删除旧
 * `phinxlog` 行，绝不执行 DDL 或改写业务表；未先升级的旧结构与未知结构同样失败关闭，必须恢复压缩前
 * 备份并用旧迁移链完成升级，禁止猜测性补列或跳过历史数据修复。
 *
 * 失败与并发：未知对象、缺少约束、外键错误、SQL 执行失败或摘要不一致均抛出异常，由 Phinx 回滚整个
 * 事务，不允许留下半个 schema 或部分迁移记录。迁移期间不得并发启动 Webman Worker。重复运行由
 * Phinx 的唯一版本记录幂等跳过。空库约创建 395 个 schema 对象，预期在本地 SSD 上数秒内完成并持有
 * SQLite schema 写锁；既有终态库不扫描媒体业务表，只校验 schema、外键并重写很小的迁移账本。
 *
 * 回滚与 MySQL：基线不可向下回滚；误操作只能恢复执行前的已验证 SQLite 备份。未来 MySQL 支持必须
 * 作为独立数据迁移重新表达类型、排序规则、CHECK、触发器和索引，不能直接执行本文件的 SQLite SQL。
 */
final class CreateVelinMusicReleaseBaseline extends AbstractMigration
{
    private const VERSION = 20260903000300;
    private const SCHEMA_FINGERPRINT = '754feca116bbfb7ceb93919eed803ee84868c928b1d648702fc46a43beb15415';
...
CREATE TABLE IF NOT EXISTS "artwork_provider_candidates" (
    id TEXT PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS "artwork_provider_import_jobs" (
    id TEXT PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS "artwork_provider_search_jobs" (
    id TEXT PRIMARY KEY NOT NULL,
    song_id TEXT NULL REFERENCES media_songs(id) ON DELETE CASCADE,
    album_id TEXT NULL REFERENCES media_albums(id) ON DELETE CASCADE,
    artist_id TEXT NULL REFERENCES media_artists(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS "media_album_artworks" (
...
CREATE TABLE IF NOT EXISTS "media_artwork_selection_overrides" (
    id TEXT PRIMARY KEY NOT NULL,
    song_id TEXT NULL REFERENCES media_songs(id) ON DELETE CASCADE,
    album_id TEXT NULL REFERENCES media_albums(id) ON DELETE CASCADE,
    artist_id TEXT NULL REFERENCES media_artists(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS "media_manual_artwork_candidates" (
    id TEXT PRIMARY KEY NOT NULL,
    song_id TEXT NULL REFERENCES media_songs(id) ON DELETE CASCADE,
    album_id TEXT NULL REFERENCES media_albums(id) ON DELETE CASCADE,
    artist_id TEXT NULL REFERENCES media_artists(id) ON DELETE CASCADE,
...
CREATE TABLE music_libraries (
    ...
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
, inbox_path TEXT NULL, resolved_inbox_path TEXT NULL, import_approval_mode TEXT NOT NULL DEFAULT 'manual' CHECK (import_approval_mode IN ('auto', 'manual')), import_stability_seconds INTEGER NOT NULL DEFAULT 60 CHECK (import_stability_seconds BETWEEN 5 AND 86400), scrape_storage_mode TEXT NOT NULL DEFAULT 'managed_cache' CHECK (scrape_storage_mode IN ('managed_cache', 'adjacent')), source_type TEXT NOT NULL DEFAULT 'local' CHECK (source_type IN ('local', 'webdav', 'onedrive', 'google_drive')), remote_metadata_mode TEXT NOT NULL DEFAULT 'filename_only'
CHECK (remote_metadata_mode IN ('filename_only', 'range_probe')), proxy_profile_id TEXT NULL
...
CREATE TABLE IF NOT EXISTS "music_sources" (
    ...
    `created_at` TEXT NOT NULL,
    `updated_at` TEXT NOT NULL
, `use_proxy` INTEGER NOT NULL DEFAULT 0, proxy_profile_id TEXT NULL
    REFERENCES network_proxy_profiles(id) ON DELETE RESTRICT);
CREATE TABLE onedrive_device_authorizations (
    ...
    consumed_at TEXT NULL,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL, proxy_profile_id TEXT NULL
    REFERENCES network_proxy_profiles(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS "user_dlna_preferences" (
    user_id TEXT PRIMARY KEY NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE media_song_deletions (
    ...
    created_at TEXT NOT NULL,
    completed_at TEXT NULL, operation_state TEXT NOT NULL DEFAULT 'idle' CHECK (operation_state IN ('idle', 'restoring', 'purging')), operation_request_id TEXT NULL, restored_at TEXT NULL, purged_at TEXT NULL,
    UNIQUE (request_id)
);
...
CREATE INDEX idx_media_song_deletions_trash ON media_song_deletions(status, purged_at, created_at);
...
CREATE INDEX idx_playlist_auto_completion_queue ON playlist_auto_completion_jobs(status, next_attempt_at, created_at);
CREATE INDEX idx_playlist_auto_completion_playlist ON playlist_auto_completion_jobs(playlist_id, status, position);
CREATE TABLE IF NOT EXISTS "playlists" (
    ...
    source TEXT NOT NULL DEFAULT 'manual' CHECK (source IN ('manual', 'lastfm', 'netease', 'qq', 'kugou')),
    source_key TEXT NULL
, auto_completion_enabled INTEGER NOT NULL DEFAULT 0 CHECK (auto_completion_enabled IN (0,1)), auto_completion_updated_at TEXT NULL);
...
CREATE TABLE IF NOT EXISTS "system_playlist_sync_rules" (
    playlist_id TEXT PRIMARY KEY NOT NULL REFERENCES playlists(id) ON DELETE CASCADE,
    provider TEXT NOT NULL CHECK (provider IN ('lastfm', 'netease', 'qq', 'kugou')),
...
CREATE TABLE metadata_sync_scrape_channel_results (
    ...
    channel_key TEXT NOT NULL CHECK (channel_key IN ('netease', 'qq', 'kugou', 'kuwo', 'migu', 'soda', 'apple_music', 'musicbrainz', 'lrclib', 'metadata-scrape')),
...
CREATE TABLE plugin_media_publications (
    ...
    published_at TEXT NULL,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL, metadata_json TEXT NULL
CHECK (metadata_json IS NULL OR (json_valid(metadata_json) = 1 AND json_extract(metadata_json, '$.schemaVersion') = 1)), discovery_etag TEXT NULL
CHECK (discovery_etag IS NULL OR length(discovery_etag) BETWEEN 1 AND 512),
    UNIQUE (plugin_key, task_id, source_fingerprint),
    UNIQUE (library_id, relative_path)
);
CREATE INDEX idx_plugin_media_publications_task ON plugin_media_publications(plugin_key, task_id, status);
CREATE INDEX idx_plugin_media_publications_library ON plugin_media_publications(library_id, published_at DESC);
...
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('apple_music', 'Apple Music', '1', '70', '["songs","albums","artwork"]', 'partial', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('kugou', '酷狗音乐', '1', '30', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('kuwo', '酷我音乐', '1', '40', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('lrclib', 'LRCLIB', '1', '90', '["songs","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('migu', '咪咕音乐', '1', '50', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('musicbrainz', 'MusicBrainz', '1', '80', '["songs","albums","artwork"]', 'available', '2', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('netease', '网易云音乐', '1', '10', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('qq', 'QQ 音乐', '1', '20', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO "music_sources" ("source_key", "display_name", "enabled", "priority", "capabilities_json", "implementation_status", "version", "created_at", "updated_at", "use_proxy", "proxy_profile_id") VALUES ('soda', '汽水音乐', '1', '60', '["songs","albums","artwork","lyrics"]', 'available', '1', '2026-08-10T00:00:00Z', '2026-08-10T00:00:00Z', '0', NULL);
INSERT INTO network_proxy_profiles VALUES('01M10GHB2PSX2AHV8XVS81FQWW','默认代理',0,'http','',7890,'',NULL,1,NULL,NULL,'2026-08-10T00:00:00Z','2026-08-10T00:00:00Z');
SQL;

    /**
     * 把旧基线创建结果推进到压缩时的精确终态。
     *
     * 这些语句只在空库路径执行，并保持原迁移的 DDL 顺序和文本形式，使 SQLite 在 `sqlite_schema`
     * 保存的定义与压缩前完整迁移链逐字等价。相对路径列和封面正文触发器仍由数据库约束兜底；空库
     * 没有历史候选或扫描任务，因此不需要重放已经在现有开发库完成的数据清洗。默认路径和设置使用
     * 幂等更新/插入，任一语句失败会随 Phinx 事务回滚整个新库。
     */
    private const TERMINAL_SQL = <<<'SQL'
UPDATE music_libraries
SET root_path = '/storage/music',
    resolved_root_path = '/storage/music',
    inbox_path = NULL,
    resolved_inbox_path = NULL,
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
WHERE source_type = 'local'
  AND root_path = '/media/library'
  AND resolved_root_path = '/media/library';

INSERT OR IGNORE INTO system_settings (setting_key, value_json, version, updated_by, updated_at)
VALUES ('resource.plugin_store', '{"sourceUrl":""}', 1, NULL, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'));

INSERT OR IGNORE INTO system_settings (setting_key, value_json, version, updated_by, updated_at)
VALUES (
    'metadata.scrape_policy',
    '{"fieldPriorities":{"title":"metadata","sortTitle":"metadata","artists":"metadata","albumArtists":"metadata","album":"metadata","trackNumber":"metadata","trackTotal":"metadata","discNumber":"metadata","discTotal":"metadata","releaseDate":"metadata","genres":"metadata","moods":"metadata","contributors":"metadata","label":"metadata","edition":"metadata","explicit":"metadata","externalIds":"metadata","lyrics":"metadata","songArtwork":"metadata","albumArtwork":"metadata"},"convertTraditionalToSimplified":false}',
    1,
    NULL,
    strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
);

CREATE TABLE media_artwork_blobs (
...
ALTER TABLE library_scan_jobs ADD COLUMN file_results_pruned_at TEXT NULL;

CREATE TRIGGER media_artwork_blob_candidate_delete
AFTER DELETE ON media_manual_artwork_candidates
BEGIN
    DELETE FROM media_artwork_blobs
    WHERE content_sha256 = OLD.content_sha256
      AND NOT EXISTS (
          SELECT 1 FROM media_manual_artwork_candidates
          WHERE content_sha256 = OLD.content_sha256
      );
END;

CREATE TRIGGER media_artwork_blob_candidate_digest_update
AFTER UPDATE OF content_sha256 ON media_manual_artwork_candidates
...
ALTER TABLE library_scan_jobs ADD COLUMN relative_path TEXT NULL
CHECK (
    relative_path IS NULL OR (
        scan_type = 'incremental'
        AND length(relative_path) BETWEEN 1 AND 768
...
SQL)->fetchColumn();
        ...
        if ($objectCount === 0) {
            ...
            $connection->exec(self::TERMINAL_SQL);
        }
...
WHERE name NOT LIKE 'sqlite_%' AND name <> 'phinxlog'
  AND NOT (
      name GLOB 'jackett_*' OR name GLOB 'lx_music_*'
      OR name GLOB 'metadata_scrape_*' OR name GLOB 'music_resource_*'
      OR name GLOB 'idx_resource_*_plugin_*'
      OR tbl_name GLOB 'jackett_*' OR tbl_name GLOB 'lx_music_*'
      OR tbl_name GLOB 'metadata_scrape_*' OR tbl_name GLOB 'music_resource_*'
  )
ORDER BY type, name
SQL)->fetchAll(PDO::FETCH_ASSOC);
        foreach ($rows as $row) {
            // Jackett 通过插件生命周期向两个共享 resource 表追加受合同约束的列；这些列不属于核心表，
            // 仅在指纹中去除，实际表结构和数据始终保留。其它插件列或未知 DDL 不会被忽略。
            $sql = $this->normalizePluginExtensions(
                (string) $row['type'],
                (string) $row['tbl_name'],
                (string) $row['sql'],
            );
            $canonical .= $row['type'] . "\x1f" . $row['name'] . "\x1f"
                . $row['tbl_name'] . "\x1f" . $sql . "\n";
        }
    ...
    /** 只移除 Jackett 合同明确允许的列，避免插件安装后的共享表扩展破坏核心指纹。 */
    private function normalizePluginExtensions(string $type, string $table, string $sql): string
    {
        if ($type !== 'table') {
            return $sql;
        }
        if ($table === 'resource_search_result_leases') {
            return (string) preg_replace(
                "/, plugin_key TEXT NOT NULL DEFAULT 'jackett'\\)$/",
                ')',
                $sql,
            );
        }
        if ($table === 'resource_download_jobs') {
            return (string) preg_replace(
                "/, plugin_key TEXT NOT NULL DEFAULT 'jackett', subscription_id TEXT NULL, subscription_item_id TEXT NULL\\)$/",
                ')',
                $sql,
            );
        }
