...
/**
 * 将已确认的批量歌词 sidecar 父方案投影为统一任务中心契约。
 *
 ...
 */
final readonly class LyricsBatchWritebackJobProjectionService
{
    ...
    public function page(
        ...
        if ($libraryId !== null) {
            $query->whereExists(function (Builder $targets) use ($libraryId): void {
                $targets->selectRaw('1')->from('lyrics_writeback_batch_targets as selected_targets')
    ...
    public function filterOptions(array $actor): array
    {
        ...
        $libraries = Db::table('lyrics_writeback_batch_targets as targets')
    ...
    {
        $query = Db::table('lyrics_writeback_batch_plans as batches')
    ...
    private function map(stdClass $row): array
    {
        ...
        return [
            ...
            'attempt' => 1,
            'error' => $failed < 1 ? null : [
                'code' => $status === 'partial' ? 'LYRICS_BATCH_PARTIAL' : 'LYRICS_BATCH_FAILED',
            ...
            'sourceDetailHref' => '/admin/media?batchId=' . rawurlencode((string) $row->id),
            'errorSamples' => null,
        ];
    }
    ...
    private function requireCapability(array $actor): void
    {
        ...
        if (!in_array('edit_metadata', $capabilities, true)) {
            throw new JobCenterInvalid('Unsupported lyrics batch writeback task type.');
        }
    }
}
