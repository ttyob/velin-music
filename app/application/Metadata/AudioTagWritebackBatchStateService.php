...
/**
 * 从单曲描述元数据音频标签任务事实幂等重建批量父方案状态。
 *
 * 本服务不读取标签值、音频文件或网络，也不领取任务。单曲 plan/job 是唯一执行事实；父批次只
 * 提供可重建的进度投影，因此 Worker 在子任务终态与父计数更新之间退出不会留下永久错误状态。
 */
final class AudioTagWritebackBatchStateService
{
    /** 同步逐项目标与父计数；draft/expired 批次不会被意外激活。 */
    public function synchronize(string $batchId): void
    {
        $schema = Db::connection()->getSchemaBuilder();
        if (!$schema->hasTable('audio_tag_writeback_batch_plans')) return;
        Db::transaction(function () use ($batchId): void {
            ...
            $targets = Db::table('audio_tag_writeback_batch_targets as targets')
                ->join('audio_tag_writeback_plans as plans','plans.id','=','targets.writeback_plan_id')
                ->leftJoin('audio_tag_writeback_jobs as jobs','jobs.plan_id','=','plans.id')
            ...
            foreach($targets as $target){[$status,$code]=$this->state($target);
                ...
                if((string)$target->target_status!==$status)Db::table('audio_tag_writeback_batch_targets')
            ...
            Db::table('audio_tag_writeback_batch_plans')->where('id',$batchId)
    ...
    private function state(stdClass $target): array
    {
        if($target->job_status!==null){$status=(string)$target->job_status;
            ...
            if(in_array($status,['failed','cancelled'],true)){$stale=(string)$target->plan_status==='stale';
                return[$stale?'stale':'failed',$target->job_error_code===null
                    ?($stale?'AUDIO_TAG_PLAN_STALE':'AUDIO_TAG_WRITEBACK_FAILED'):(string)$target->job_error_code];}
        ...
        if((string)$target->plan_status==='stale')return['stale',$target->plan_error_code===null
            ?'AUDIO_TAG_PLAN_STALE':(string)$target->plan_error_code];
